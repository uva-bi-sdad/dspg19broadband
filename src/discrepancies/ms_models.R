...
discr <- discr %>% filter(!is.na(usage))
miss_var_summary(discr)

# Plot outcome & key predictor
hist(discr$availability_adv)
hist(discr$usage)
...
# Sample model
n_features <- length(setdiff(names(data_train), "availability_adv"))
...
comparison <- cbind(availability_adv = data_test$availability_adv, preds = preds$preds.predictions)
...
# Sample model
n_features <- length(setdiff(names(data_train_urban), "availability_adv"))
...
comparison <- cbind(availability_adv = data_test_urban$availability_adv, preds = preds$preds.predictions)
comparison <- as.data.frame(comparison)
head(comparison)
...
# Sample model
n_features <- length(setdiff(names(data_train_rural), "availability_adv"))
...
comparison <- cbind(availability_adv = data_test_rural$availability_adv, preds = preds$preds.predictions)
comparison <- as.data.frame(comparison)
head(comparison)
...
discr_m2 <- lm(availability_adv ~ ., data = data_train)

# Predict
preds <- predict(discr_m2, data_test)
preds <- data.frame(preds)

comparison <- cbind(availability_adv = data_test$availability_adv, preds = preds)
comparison <- as.data.frame(comparison)
head(comparison)
...
discr_m2 <- lm(availability_adv ~ ., data = data_train_urban)

# Predict
preds <- predict(discr_m2, data_test_urban)
preds <- data.frame(preds)

comparison <- cbind(availability_adv = data_test_urban$availability_adv, preds = preds)
comparison <- as.data.frame(comparison)
head(comparison)
...
discr_m2 <- lm(availability_adv ~ ., data = data_train_rural)

# Predict
preds <- predict(discr_m2, data_test_rural)
preds <- data.frame(preds)

comparison <- cbind(availability_adv = data_test_rural$availability_adv, preds = preds)
comparison <- as.data.frame(comparison)
head(comparison)
