...
df <- as.data.frame(cbind(years, links))
...
fcc_history <- function(year){
  ...
  if (!file.exists(destzip)) {
    download.file(url = df$links[df$year == year],
                  destfile = destzip, mode = 'wb')
  }
  unzip(destzip, exdir = destfolder)
}
  
# fcc_history(2014) <- Warning message:
#In unzip(destzip, exdir = destfile) : zip file is corrupt

# fcc_history(2015)

# the zips are corrupted - try diff versions
