...
acs_fcc_shapes <- function(state, geography, r_u){
  ...
  if(geography == "Block Group"){
    acs_file <- here("rivanna_data", "dashboard", "summary_acs.csv")
    acs <- fread(acs_file, colClasses=c(state = "character", county = "character", census_tract = "character", block_group = "character"))
    fcc_file <- here("rivanna_data", "dashboard", "fcc_processed_25.csv")
    ...
    acs_file <- here("rivanna_data", "dashboard", "summary_acs_census_tract.csv")
    acs <- fread(acs_file, colClasses = c(state = "character", county = "character", census_tract = "character"))
    fcc_file <- here("rivanna_data", "dashboard", "fcc_processed_tract_25.csv")
...
county_shapes <- function(state, r_u){
  state_fips = usmap::fips(state)
  mic <- here("rivanna_data", "dashboard", 'microsoft.csv')
  ...
  # Merge with FCC
  fcc_data <- here("rivanna_data", "dashboard", "fcc_processed_county_25.csv")
...
server <- function(input, output, session){
  ...
  definitions_file <- here("rivanna_data", "dashboard", "definitions_key.csv")
...
ui <- fluidPage(
  ...
  headerPanel(img(src = 'logo.png', class = 'topimage', width = '30%', style = 'display: block; margin-left: auto; margin-right: auto;')),
  
  fluidRow(width = 12, 
           style = "padding-top:20px; padding-bottom:20px;",
           column(12, align = 'center', 
                  h1(strong('US Broadband Coverage Discrepancy Map')))
  ),
  ...
  fluidRow(style = "margin: 6px; padding-top:20px; padding-bottom:20px",  
           tabsetPanel(
             tabPanel(title = "Broadband Data Comparison Maps",
                      
                      fluidRow(width = 12, style = "margin: 6px",
                               column(12, align = "left", 
                               h3(strong('Broadband Data Comparison Maps'))
                               )
                      ),
                     ...
                     FCC broadband availability and ACS broadband subscription comparison.')
                               ...
                               column(10, 
                                      h4('Map'),
                                      leafletOutput("mymap", height = 550, width = "100%")
                               )
                      ),
             ...
             tabPanel(title = "More on Connectivity",
                      ...
                      fluidRow(style = "margin: 6px",
                               ...
                               column(8,
                                    ...
                                    based on conditions in their counties. Maps, details about our data sources, and measure descriptions are available on the dashboard website.")
                                      )
                               ),
                               column(4,
             ...
             tabPanel(title = "Acknowledgments and Contact",
                      ...
                      fluidRow(width = 12, style = "margin: 20px",
                               column(6, align = 'center',
                                      wellPanel(
                                        h4('Acknowledgments'),
                                        br('We would like to thank our 2019 Data Science for Public Good program participants -- ', a(href = 'https://www.linkedin.com/in/kateryna-savchyn/', 'Kateryna Savchyn,'), a(href = 'https://www.linkedin.com/in/sarahmcdnld', 'Sarah McDonald,'), 'and ', a(href = 'https://www.linkedin.com/in/raghavsawhney96/', 'Raghav Sawhney'), '-- for their valuable contributions to this project.')
                                      )
                               ),
                               column(6, align = 'center',
                                      wellPanel(
                                        h4('Contact'),
                                        p(a(href = 'https://biocomplexity.virginia.edu/joshua-goldstein', 'Joshua Goldstein, PhD'), 'and', a(href = 'https://biocomplexity.virginia.edu/teja-pristavec', 'Teja Pristavec, PhD')),
                                        p('University of Virginia, Biocomplexity Institute and Initiative'),
                                        p(a(href = 'https://biocomplexity.virginia.edu/social-decision-analytics', 'Social and Decision Analytics Division', target = '_blank'))
                                      )
                               )
                      )
             )
  ...
  fluidRow(width = 12, style = "margin: 20px",
           column(12, align = 'center',
                  ...
                  br(tags$small(em('Last updated: June 2020'))))
  )
)
